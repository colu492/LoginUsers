...
class ProductManagerDB {
    ...
    addProduct = async ({
        ...
        (!title || title.length === 0) &&
            errortxt.push("este campo es obligatorio.");
        (!description || description.length === 0) &&
            errortxt.push("este campo es obligatorio.");
        (!code || code.length === 0) && errortxt.push("este campo es obligatorio.");
        ...
        try {
            const found = await productModel
                ...
                .exec();
            if (found !== null) {
                errortxt.push("Ya hay un producto con el mismo code.");
            }
        } catch (error) {
            return { error: 3, servererror: error };
        }
    ...
    updateProduct = async ({
        ...
        try {
            ...
            (!status || status.length === 0) &&
                errortxt.push("este campo es obligatorio.");
            !thumbnails && errortxt.push("este campo es obligatorio.");
            !Array.isArray(thumbnails) &&
                errortxt.push("thumbnails tiene que ser un array.");
            ...
            const codefound = await productModel
                ...
                .exec();
            if (codefound !== null) {
                errortxt.push("Ya hay producto con el mismo code.");
            }
